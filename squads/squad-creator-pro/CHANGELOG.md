...
All notable changes to the squad-creator-pro pack will be documented in this file.
...
Based on Executor Decision Tree analysis (`*optimize squad-creator-pro`), converted deterministic tasks from Agent (LLM) to Worker (Python scripts):
...
### Fixed
- config.yaml name mismatch (squad-chief → squad-creator-pro)
...
- Additional specialist agents (legal-squad-creator-pro, copy-squad-creator-pro)
