...
|------------|--------|------------|
| `@oalanicolas-pro` | Mind cloning, DNA extraction, source curation | `/squad-creator-pro @oalanicolas-pro` |
| `@pedro-valerio-pro` | Processes, tasks, checklists, automation | `/squad-creator-pro @pedro-valerio-pro` |
| `@squad-chief-pro` | General squad creation, orchestration | `/squad-creator-pro` (default) |
  ...
  decision_tree: |
    ...
    TERTIARY - General Tasks:
      IF (task is general squad creation)
        THEN use @squad-chief-pro
      IF (unclear which specialist)
        THEN use @squad-chief-pro (will delegate)

    FALLBACK:
      When in doubt → @squad-chief-pro orchestrates
...
anti_patterns:
  - name: "Wrong Specialist"
    trigger: "Using @pedro-valerio-pro for voice extraction"
    why_bad: "Process expert, not mind cloning expert"
    correction: "Use @oalanicolas-pro for DNA extraction"

  - name: "Skipping Specialists"
    trigger: "Trying to do everything with @squad-chief-pro"
